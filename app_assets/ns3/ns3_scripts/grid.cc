...
MeshTest::InstallApplication ()
{
  ...
  for (int i=1; i < (m_xSize*m_ySize - 1); i++) {
    Ptr<Node> node = nodes.Get (i);
    Vector p = node->GetObject<MobilityModel>()->GetPosition();
    std::fprintf(fp, "%d\t%f\t%f\n", i, p.x, p.y);
  }
  Ptr<Node> node = nodes.Get (m_xSize*m_ySize - 1);
  Vector clientp = node->GetObject<MobilityModel>()->GetPosition();
  std::fprintf(fp, "%d\t%f\t%f\tCLIENT\n", m_xSize*m_ySize - 1, clientp.x, clientp.y);
  std::fclose(fp);
}
int
MeshTest::Run ()
